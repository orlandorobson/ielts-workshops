...
const releases=createReleaseControl(releaseConfig);
let releaseUI;
...
function render(){app.querySelectorAll('audio').forEach(a=>a.pause());state.step=Number.isInteger(state.step)?Math.max(0,Math.min(state.step,screens.length-1)):0;const s=screens[state.step];const stage=releases.stageForScreen(state.step);releaseUI.update(stage);if(!releases.isOpen(stage.id)){document.querySelector('#progress-label').textContent=`Day 1 · Stage ${releaseConfig.stages.indexOf(stage)+1} · Locked`;document.querySelector('#progress').value=releaseConfig.stages.indexOf(stage)+1;app.innerHTML=releaseUI.locked(stage);save();return;}document.querySelector('#progress-label').textContent=`Day 1 · Stage ${releaseConfig.stages.indexOf(stage)+1} of 6 · ${s.label}`;document.querySelector('#progress').value=releaseConfig.stages.indexOf(stage)+1;app.innerHTML=`<p class="eyebrow">${s.label}</p><h1>${s.title}</h1>${s.body()}<nav class="nav" aria-label="Activity navigation">${state.step?'<button class="back" data-back>Back</button>':''}<button class="primary" data-next>${s.next}</button></nav>`;save();}
function move(delta){const current=releases.stageForScreen(state.step);const target=releases.stageForScreen(state.step+delta);if(delta>0&&current.id!==target.id)releases.complete(current.id);state.step+=delta;if(screens[state.step]?.section===7&&!state.answers.action && state.answers.change)state.answers.action=[...state.answers.change];render();document.querySelector('#activity').focus();window.scrollTo(0,0);}
app.addEventListener('change',e=>{const t=e.target;if(!t.matches('input,select')||t.hasAttribute('data-release-code'))return;state.answers[t.name]=t.tagName==='SELECT'?(t.value===''?[]:[Number(t.value)]):Array.from(app.querySelectorAll(`input[name="${t.name}"]:checked`),x=>Number(x.value));if(t.name==='action')document.querySelector('#action-feedback').textContent=actionText();if(t.name==='focus')document.querySelector('#focus-feedback').innerHTML=focusText();if(t.name==='faster')document.querySelector('#feedback').textContent='Not necessarily. Good fluency does not mean speaking as fast as possible.';if(state.checked.listening&&document.querySelector('#listening-feedback'))document.querySelector('#listening-feedback').innerHTML=listeningFeedback();save();});
app.addEventListener('click',e=>{const b=e.target.closest('button');if(!b)return;if(b.hasAttribute('data-next')){if(state.step===screens.length-1){state.complete=true;releases.complete(releases.stageForScreen(state.step).id);releaseUI.update(releases.stageForScreen(state.step));save();app.innerHTML='<p class="eyebrow">Day 1 complete</p><h1>Listen to yourself.<br>Change one thing.<br>Try again.</h1>'+feedback(focusText())+'<nav class="nav"><button data-review>Review Day 1</button></nav>';document.querySelector('#activity').focus();window.scrollTo(0,0);}else move(1);}if(b.hasAttribute('data-back'))move(-1);if(b.hasAttribute('data-review')){state.step=0;render();window.scrollTo(0,0);}if(b.hasAttribute('data-check-listening')){state.checked.listening=true;document.querySelector('#listening-feedback').innerHTML=listeningFeedback();save();}});
...
releaseUI=installStudentRelease({control:releases,app,progress:document.querySelector('.progress-wrap'),onJump:index=>{state.step=index;render();document.querySelector('#activity').focus();window.scrollTo(0,0);},onRefresh:render});
const requestedStage=releaseConfig.stages.find(s=>s.id===new URLSearchParams(location.search).get('stage'));
if(requestedStage)state.step=requestedStage.firstScreen;
bindAudio(app);
render();
